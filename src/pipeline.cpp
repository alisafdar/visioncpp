...
// Embedded symbols generated by xxd via embed_resources.cmake
extern "C" {
...
namespace vision {
    ...
    // YUV_420_888 (3 planes) -> RGBA (no rotation)  [internal helper]
    static cv::Mat yuv420ToRgba(const uint8_t* y, const uint8_t* u, const uint8_t* v,
        ...
        // Copy Y plane (account for row stride)
        for (int r = 0; r < height; ++r) {
            std::memcpy(i420.data() + r * width, y + r * yRowStride, width);
        }
        // De-interleave U/V planes using pixelStride
    ...
    // Rotate an RGBA Mat by 0/90/180/270 (clockwise)
    static inline void rotateRgbaInPlace(cv::Mat& img, int rotationDeg) {
        const int r = ((rotationDeg % 360) + 360) % 360;
        if (r == 90) {
            cv::transpose(img, img);
            cv::flip(img, img, 1);  // transpose + flip(1) = CW
        } else if (r == 270) {
            cv::transpose(img, img);
            cv::flip(img, img, 0);  // transpose + flip(0) = CCW
        } else if (r == 180) {
            cv::flip(img, img, -1);
        }
        // 0 deg: no-op
    ...
    Detections pipelineProcessYuvRotated(const YuvFrame& f, const ProcessConfig& cfg, int rotationDeg) {
        ...
        // 5) Detection on rotated frame
        //    tfliteDetect returns pixel-space boxes relative to 'rgba' size.
        ...
        for (const auto& d : det) {
            // Ensure order [left, top, right, bottom] in pixels
    ...
    // Backward-compatible entry: no rotation (calls rotated variant with 0 deg)
    Detections pipelineProcessYuv(const YuvFrame& f, const ProcessConfig& cfg) {
        return pipelineProcessYuvRotated(f, cfg, /*rotationDeg=*/0);
    }
